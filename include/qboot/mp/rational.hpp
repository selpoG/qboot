...
namespace qboot::mp
{
	...
	public:
		...
		explicit rational(std::string_view o) : rational()
		{
			...
			{
				mpq_clear(_x);
				throw std::runtime_error("in qboot::mp::rational(string_view):\n  invalid input format "s += o);
			}
			mpq_canonicalize(_x);
		}
...
#pragma GCC diagnostic pop
		...
		friend mpq_srcptr _mp_ops<rational>::data(const rational& rop);
		friend rational _mp_ops<rational>::get(mpfr_srcptr rop, mpfr_rnd_t rnd);
	...
	inline rational approx(const rational& x, const rational& error, bool use_relerr = false)
	{
		...
		for (uint32_t i = 1; i < cf.size() && qboot::mp::abs(a * q - p * b) >= q * e; ++i)
		{
			auto np = cf[i] * p + p1;
			auto nq = cf[i] * q + q1;
			p1 = p;
