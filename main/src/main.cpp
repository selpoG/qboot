...
using qboot::h_asymptotic, qboot::gBlock_full, qboot::Context, qboot::PrimaryOperator;
using std::array;
...
int main()
{
	...
	auto c2 = qboot2::context_construct(n_Max, R::prec, lambda);
	{
		...
		std::cout << c.F_minus_matrix(R(0.7)) * gBlock_full(op, R(0), R(0)) << std::endl;
		std::cout << c.rho_to_z_matrix << std::endl;
